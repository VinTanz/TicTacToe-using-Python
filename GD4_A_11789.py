# Library yang digunakan untuk membuat tic tac toe 
import copy
import sys
import pygame
import random
import numpy as np # Library yang digunakan untuk perhitungan matriks peta

from constants import * # Variabel atau tetapan yang digunakan di file constants.py
from tkinter import messagebox
import tkinter as tk

# --- PYGAME SETUP ---
pygame.init() # Inisialisasi Game Awal
screen = pygame.display.set_mode((WIDTH, HEIGHT)) # Tampilan Peta dari tic tac toe
pygame.display.set_caption('TIC TAC TOE AI') # Tampilan Caption Tic Tac Toe
screen.fill( BG_COLOR ) # Warna Background Dari Peta Tic Tac Toe 

# --- CLASSES ---
class Board :
    def __init__(self) : # Inisialisasi Awal Board
    ...
    def final_state(self, show=False):
        ...
        '''
        # Vertical Wins
        for col in range(COLS): # Jika Kemenangan Diperoleh Secara Vertikal 
            if self.squares[0][col] == self.squares[1][col] == self.squares[2][col] != 0: # Menandakan Bahwa Kemenangan Terjadi Secara Vertikal
                if show:
                    color = CIRC_COLOR if self.squares[0][col] == 2 else CROSS_COLOR  # Menampikan Warna Ketika Kemenangan Vertikal
                    iPos = (col * SQSIZE + SQSIZE // 2, 20) 
                    fPos = (col * SQSIZE + SQSIZE // 2, HEIGHT - 20)
                    pygame.draw.line( screen, color, iPos, fPos, LINE_WIDTH) # Menggambar Garis Kemenangan Vertikal
                return self.squares[0][col]
            
        # Horizontal Wins
        for row in range(ROWS): # JIka Kemenangan Diperoleh Secara Horizontal
...
class Game:
    def __init__(self): # Inisialiasi Awal Dari Board 
        ...
        self.running = True # Inisialisasi AI menjadi self-running
    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:# Logika keyboard ketika ingin mengubah gamemode atau melakukan reset
                ...
                # 0 - random ai
                if event.key == pygame.K_0: # Logika Mengubah Level dari AI
                    ai.level = 0
                # 1 - random ai
                if event.key == pygame.K_1: # Logika Mengubah Level dari AI
                    ai.level = 1
            if event.type == pygame.MOUSEBUTTONDOWN: # Logika Click Event 
                pos = event.pos
                row = pos[1] // SQSIZE
                col = pos[0] // SQSIZE

                # Human Mark SQR
                if board.empty_sqr(row, col) and game.running: # Menerima Inputan Dari Player terhadap Board
                    game.make_move(row, col) # Logika Pengaturan Gerakan Player
                    if game.isover(): # Pengecekan Apakah Game Sudah Berakhir
                        game.running = False
        # AI initial call untuk memanggil Fungsi Pergerakan AI
        if game.gamemode == 'ai' and game.player == ai.player and game.running:
            pygame.display.update() # Melakukan Update Pergerakan AI
            row, col = ai.eval(board)
            game.make_move(row, col)
            if game.isover(): # Pengecekan Apakah Game Sudah Berakhir
                game.running = False
            
        pygame.display.update() # Pemanggilan Fungsi untuk Update Tampilan
